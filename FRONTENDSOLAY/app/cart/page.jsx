				...
				<div className="max-w-3xl mx-auto">
					...
					<div className="mt-8">
						<ul className="space-y-4">
							{cart?.map((item) => {
								    const imageUrl = item?.product?.attributes?.foto?.data?.attributes?.url 
									? `https://solay-backend-420j.onrender.com${item.product.attributes.foto.data.attributes.url}` 
									: '';
								...
								return (
									<li key={item.id} className="flex items-center gap-4">
										...
										<div className="flex items-center justify-end flex-1 gap-2">
											TL{item?.product?.attributes?.price}
